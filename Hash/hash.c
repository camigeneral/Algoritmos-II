...
bool hash_redimensionar(hash_t **hash, size_t nueva_capacidad) {
    ...
    for (int i = 0; i < (*hash)->capacidad; i++) {
        ...
        while (!lista_iter_al_final(iter)) {
            ...
            size_t clave_hasheada = jenkins_hash((unsigned char*)dato->clave, strlen((dato->clave))) % nueva_capacidad; 
...
bool hash_guardar(hash_t *hash, const char *clave, void *dato) {
    ...
    size_t clave_hasheada = jenkins_hash((unsigned char*)clave, strlen(clave)) % hash->capacidad;
